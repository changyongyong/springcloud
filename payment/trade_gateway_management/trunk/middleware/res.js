...
module.exports = (req, res, next) => {
    ...
    res.dtResponse = function (data, options) {
        if (!options) {
            options = {};
        }
        if (!data) {
        ...
        var result;
        result = {
            tag: options.error && 'error' || 'success',
            statue: options.error && DEFAULT_ERROR_STATUS || DEFAULT_SUCCESS_STAUS,
            error: options.error || '',
            draw: req.query.draw || req.body.draw || 0,
            data: data || [],
            recordsTotal: options.count || 0,
            recordsFiltered: options.recordsFiltered || options.count || 0,
            length: data.length
        };
        for (var key in options) {
            if (options.hasOwnProperty(key) && !_.includes(['count', 'error'], key)) {
