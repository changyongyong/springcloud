...
router.get('/record', joiValidator({
    query: {
        ...
        orderType: Joi.valid(Mapping.TRANSFER_ORDER_TYPES_ARR, 'other'),
        tradeRecordNo: Joi.string(),
        cityId: Joi.string(),
        _: Joi.any()
    }
}), (req, res) => {
    Transfer.records(req.query)
        .then((data) => {
...
router.get('/record/export', joiValidator({
    ...
    Transfer.records(req.query)
        .then((data) => {
            ...
            conf.rows = result;
            var filename = '转账记录' + moment().format('YYYY-MM-DD-HH-mm-ss') + '.xlsx';
...
router.get('/record/statistic', joiValidator({
    ...
    Statistics.transferRecordStatistics(req.query)
        .then(data => {
            res.success(data);
        })
});
