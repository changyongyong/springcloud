...
const paymentRecordFormat = function (data) {
    let result = {
        ...
        status: data.status,
        statusStr: Mapping.STATUS_TRANS[data.status] || '未知',
        ...
        orderTypeStr: Mapping.PAYMENT_ORDER_TYPES_TRANS[data.orderType] || '商城退款',
...
module.exports.records = function (par) {
    ...
    if (orderNo && orderNo != 0) {
        where.paymentRecordNo = orderNo;
    }
    limit = parseInt(limit);
    offset = parseInt(offset);
    // let where = {};
    return RefundRecordDb.findAndCountAll({
        where: where,
        limit: limit,
        offset: offset,
        raw: true,
