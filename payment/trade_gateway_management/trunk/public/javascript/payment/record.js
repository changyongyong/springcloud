...
Record.prototype.init = function () {
    ...
    options = {
        ...
        columns: [{
            ...
            {
                data: 'refundStatusStr',
                width: '7%',
                title: '退款状态',
                createdCell: function (cell, cellData, data) {
                    switch (data.refundStatus) {
                        case 'NONE':
                            $(cell).addClass('payment-refund-none');
                            break;
                        case 'ALL':
                            $(cell).addClass('payment-refund-all');
                            break;
                        case 'PART':
                            $(cell).addClass('payment-refund-part');
                            break;
                    }
                }
            },
    ...
    $('[func=export]').on('click', function () {
        var url = '/api/payment/record/export?';
...
Record.prototype.statisticsInit = function () {
    ...
    var query = that.query;
    $.ajax({
        url: '/api/payment/record/statistic',
        method: 'GET',
        data: query,
