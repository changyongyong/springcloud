const { MqSendMessage } = require('../../models/wxdb');
const MESSAGE_ID_START = 'WE_CHAT_PAY_TUNNEL_';
...
Publisher.prototype.initMessage = function (message) {
    // 系统开头+时间戳+进程号+2位随机数
    const no = MESSAGE_ID_START + Date.now() + '_' + PROCESS_ID + '_' + parseInt(Math.random() * 100);
